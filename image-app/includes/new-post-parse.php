...
if( $_POST['did_upload'] ){
	...
	if($valid){
		...
		// if it worked, Add post to Database
		if($did_save){
			$user_id = $logged_in_user['user_id'];
			echo $sql = "INSERT INTO posts
					(title, image, date, user_id, body, allow_comments, is_published, category_id)
					VALUES
					('', '$unique_name', now(), $user_id, '', 0, 0, 0)";
			$result = $db->query($sql);
			if( !$result ){
				echo $db->error;
			}
			if( $db->affected_rows >= 1 ){
				//success! send the user to step 2
				//$feedback = 'Successfully created post.';
				//$feedback_class = 'success';
				//get the primary key of the post we just created
